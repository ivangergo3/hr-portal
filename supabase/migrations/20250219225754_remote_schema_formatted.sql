-- Supabase Migration: 20250219225754_remote_schema.sql
...
CREATE OR REPLACE FUNCTION "public"."update_timesheet_week_total_hours"()
RETURNS "trigger"
LANGUAGE "plpgsql"
AS $$
BEGIN
...
-- Triggers
CREATE OR REPLACE TRIGGER update_timesheet_week_total_hours
  AFTER INSERT OR DELETE OR UPDATE ON "public"."timesheets"
  FOR EACH ROW EXECUTE FUNCTION "public"."update_timesheet_week_total_hours"();
...
ALTER TABLE ONLY "public"."time_off_requests"
  ADD CONSTRAINT time_off_requests_user_id_fkey FOREIGN KEY (user_id)
  REFERENCES "public"."users"(id) ON DELETE CASCADE;

ALTER TABLE ONLY "public"."timesheet_weeks"
  ADD CONSTRAINT timesheet_weeks_approved_by_fkey FOREIGN KEY (approved_by)
  REFERENCES "public"."users"(id);

ALTER TABLE ONLY "public"."timesheet_weeks"
  ADD CONSTRAINT timesheet_weeks_user_id_fkey FOREIGN KEY (user_id)
  REFERENCES "public"."users"(id);

ALTER TABLE ONLY "public"."timesheets"
  ADD CONSTRAINT timesheets_approved_by_fkey FOREIGN KEY (approved_by)
  REFERENCES "public"."users"(id);

ALTER TABLE ONLY "public"."timesheets"
  ADD CONSTRAINT timesheets_client_id_fkey FOREIGN KEY (client_id)
  REFERENCES "public"."clients"(id) ON DELETE RESTRICT;

ALTER TABLE ONLY "public"."timesheets"
  ADD CONSTRAINT timesheets_project_id_fkey FOREIGN KEY (project_id)
  REFERENCES "public"."projects"(id) ON DELETE RESTRICT;

ALTER TABLE ONLY "public"."timesheets"
  ADD CONSTRAINT timesheets_user_id_fkey FOREIGN KEY (user_id)
  REFERENCES "public"."users"(id) ON DELETE CASCADE;

ALTER TABLE ONLY "public"."timesheets"
  ADD CONSTRAINT timesheets_week_id_fkey FOREIGN KEY (week_id)
  REFERENCES "public"."timesheet_weeks"(id);
...
CREATE POLICY "Admins can do everything with timesheets" ON "public"."timesheets"
  TO "authenticated"
  USING (
    EXISTS (
      SELECT 1 FROM "public"."users"
...
CREATE POLICY "Admins can update any time off request" ON "public"."time_off_requests"
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM "public"."users"
      WHERE users.id = auth.uid() AND users.role = 'admin'::text
...
CREATE POLICY "Admins can update weekly timesheet status" ON "public"."timesheet_weeks"
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM "public"."users"
      WHERE users.id = auth.uid() AND users.role = 'admin'::text
...
CREATE POLICY "Admins can view all time off requests" ON "public"."time_off_requests"
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM "public"."users"
      WHERE users.id = auth.uid() AND users.role = 'admin'::text
...
CREATE POLICY "Admins can view all weekly timesheets" ON "public"."timesheet_weeks"
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM "public"."users"
      WHERE users.id = auth.uid() AND users.role = 'admin'::text
...
CREATE POLICY "Employees can view non-archived projects" ON "public"."projects"
  FOR SELECT TO "authenticated"
  USING (
    EXISTS (
      SELECT 1 FROM "public"."users"
      WHERE users.id = auth.uid() AND users.role = 'employee'::text
    ) AND NOT archived
  );

CREATE POLICY "Enable insert access for all authenticated users" ON "public"."users"
  FOR INSERT TO "authenticated"
  WITH CHECK (true);
...
CREATE POLICY "Users can insert their own weekly timesheets" ON "public"."timesheet_weeks"
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can manage their own draft timesheets" ON "public"."timesheets"
  TO "authenticated"
  USING (user_id = auth.uid() AND status = 'draft'::text);

CREATE POLICY "Users can update their own draft requests" ON "public"."time_off_requests"
  FOR UPDATE USING (auth.uid() = user_id AND status = 'draft'::text)
  WITH CHECK (auth.uid() = user_id AND status = 'draft'::text);

CREATE POLICY "Users can update their own weekly timesheets" ON "public"."timesheet_weeks"
  FOR UPDATE USING (auth.uid() = user_id AND status = ANY (ARRAY['draft'::text, 'pending'::text]));

CREATE POLICY "Users can view their own time off requests" ON "public"."time_off_requests"
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can view their own time off requests and admins can view" ON "public"."time_off_requests"
  FOR SELECT TO "authenticated"
  USING (
...
CREATE POLICY "Users can view their own timesheets and admins can view all" ON "public"."timesheets"
  FOR SELECT TO "authenticated"
  USING (
    user_id = auth.uid() OR EXISTS (
      SELECT 1 FROM "public"."users"
...
CREATE POLICY "Users can view their own weekly timesheets" ON "public"."timesheet_weeks"
  FOR SELECT USING (auth.uid() = user_id);
...
-- Publication and Schema Permissions
...
GRANT ALL ON FUNCTION "public"."get_dashboard_metrics"(
  "start_date" TIMESTAMP WITHOUT TIME ZONE,
  "end_date" TIMESTAMP WITHOUT TIME ZONE
) TO "anon";
GRANT ALL ON FUNCTION "public"."get_dashboard_metrics"(
