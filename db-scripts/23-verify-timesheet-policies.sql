...
-- Recreate policies
CREATE POLICY "Users can view their own timesheets" ON public.timesheets FOR ALL USING (auth.uid () = user_id);

CREATE POLICY "Admins can view all timesheets" ON public.timesheets FOR ALL USING (
  EXISTS (
    SELECT
      1
    FROM
