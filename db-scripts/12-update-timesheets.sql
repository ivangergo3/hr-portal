...
create policy "Users can manage their own draft timesheets" on public.timesheets for all to authenticated using (
  user_id = auth.uid ()
  and status = 'draft'
);
...
create policy "Admins can do everything with timesheets" on public.timesheets for all to authenticated using (
  exists (
    select
      1
    from
